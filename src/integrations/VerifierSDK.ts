...
import { computeHash, canonicalize } from '../canonicalization';
 ...
 */
export function validateTranscript(session: VerifierTranscriptSession): VerificationResult {
  const errors: string[] = [];
 ...
 * Pure function — no side effects
 */
export function verifyEntryHashes(
  session: VerifierTranscriptSession,
  errorCollector?: string[]
): boolean {
  const errors = errorCollector || [];
 ...
 * Pure function — no side effects
 */
export function computeEntryHash(entry: VerifierTranscriptEntry): string {
 ...
 * Pure function — no side effects
 */
export function getTranscriptHead(session: VerifierTranscriptSession): string | null {
  if (!session.entries || session.entries.length === 0) {
    return null;
  }
 ...
 * Pure function — no side effects
 */
export function verifyTranscriptHead(
  session: VerifierTranscriptSession,
