 ...
 */
export function extractSessionOutcome(session: VerifierTranscriptSession): SessionOutcome {
 ...
 * Pure function — no side effects
 */
export function calculateBaseReputation(outcome: SessionOutcome): number {
  switch (outcome) {
    case 'success':
      return 100;
    case 'rejected':
...
export function calculateVolume(
  ...
  const completedCount = sessions.filter(
    s => extractSessionOutcome(s) !== 'pending'
  ).length;
 ...
 * Pure function — no side effects
 */
export function calculatePolicyCompliance(
  sessions: VerifierTranscriptSession[]
): ReputationFactor {
  if (sessions.length === 0) {
 ...
 * Pure function — no side effects
 */
export function compareReputation(
