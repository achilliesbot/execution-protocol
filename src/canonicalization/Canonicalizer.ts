 ...
 */
export function validateDeterminism(obj: unknown): void {
...
export function computeProposalHash(
  proposal: Record<string, unknown> | unknown,
  ...
  const proposalId = (proposal as any)?.proposal_id;
 ...
 */
export function computePolicyHash(policySet: Record<string, unknown>): string {
  return computeHash({
    type: 'policy_set',
