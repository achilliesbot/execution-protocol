...
import { TranscriptLogger } from '../transcript/TranscriptLogger';
...
export interface SessionConfig {
  ...
  maxLossPercent: number; // e.g., 0.02 = 2%
  approvalRequired: boolean;
  simulationMode: boolean; // Phase 1: ALWAYS true
}
...
export interface SessionState {
  ...
  hash: string; // Current state hash for verification
}
...
export interface ExecutionStep {
  ...
  output?: any;
  timestamp: number;
  hash: string; // Step hash for verification chain
  approved?: boolean;
  approver?: string;
}
...
export class DeterministicSession {
   ...
   */
  async execute(): Promise<SessionState> {
   ...
   */
  private async executeStep(type: ExecutionStep['type'], fn: () => Promise<any>): Promise<any> {
   ...
   */
  private async validateConstraints(): Promise<boolean> {
  ...
  private async loadCheckpoint(): Promise<any | null> {
    try {
      const fs = require('fs').promises;
      const path = `/data/.openclaw/workspace/execution-protocol-v2/sessions/${this.sessionId}_checkpoint.json`;
