 ...
 */
export const VALID_ATTESTATION_NETWORKS = ['none', 'devnet', 'testnet'] as const;
...
export function isAttestationEnabled(): boolean {
  return (
    PHASE5_FEATURES.ATTESTATION_ENABLED &&
    PHASE5_FEATURES.ATTESTATION_NETWORK !== 'none' &&
    VALID_ATTESTATION_NETWORKS.includes(PHASE5_FEATURES.ATTESTATION_NETWORK as AttestationNetwork)
  );
}
 ...
 */
export function isFeeAccountingEnabled(): boolean {
 ...
 */
export function validatePhase5Config(): void {
  // Verify attestation network is valid
  if (PHASE5_FEATURES.ATTESTATION_NETWORK !== 'none') {
    if (!VALID_ATTESTATION_NETWORKS.includes(PHASE5_FEATURES.ATTESTATION_NETWORK as AttestationNetwork)) {
      throw new Error(
        `Invalid ATTESTATION_NETWORK: ${PHASE5_FEATURES.ATTESTATION_NETWORK}. ` +
 ...
 */
export function killSwitchPhase5(): void {
  (PHASE5_FEATURES as any).ATTESTATION_ENABLED = false;
  (PHASE5_FEATURES as any).FEE_ACCOUNTING_ENABLED = false;
  (PHASE5_FEATURES as any).ATTESTATION_NETWORK = 'none';
