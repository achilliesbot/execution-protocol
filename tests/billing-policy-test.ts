...
import {
  ...
  compareBillingResults,
  DEFAULT_BILLING_POLICY,
  BillingResult
} from '../src/billing';

import { ExecutionMetrics } from '../src/fees';

// Test utilities
function createTestMetrics(complexityScore: number = 250): ExecutionMetrics {
  return {
...
function testDeterministicBilling(): boolean {
...
function testRoundingDeterminism(): boolean {
  const metrics1 = createTestMetrics(123.456 as any); // Would be integer in reality
  ...
  return (
    Number.isFinite(result1.billable_units) &&
...
function testRejectsNegative(): boolean {
  const badMetrics: ExecutionMetrics = {
    ...createTestMetrics(),
...
// Test 4: Rejects NaN
function testRejectsNaN(): boolean {
  const badMetrics: ExecutionMetrics = {
    ...createTestMetrics(),
...
function testRejectsInfinity(): boolean {
  const badMetrics: ExecutionMetrics = {
    ...createTestMetrics(),
...
function testBillingResultValidation(): boolean {
  const valid: BillingResult = {
    session_id: 'test',
  ...
  return (
    validateBillingResult(valid) === true &&
...
function testTriggerConditions(): boolean {
...
function testWeightAffectsTotal(): boolean {
...
function testTimestampExcluded(): boolean {
...
function testPricingVersionFormat(): boolean {
...
// Run all tests
export function runBillingPolicyTests(): void {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
