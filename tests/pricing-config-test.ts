...
function cleanupTestPricing(): void {
...
function testPricingHashStability(): boolean {
...
function testVersionFileImmutability(): boolean {
  cleanupTestPricing();
...
function testDeterministicParse(): boolean {
...
function testValidatePricingConfig(): boolean {
  // Valid config
  ...
  return (
    valid === true &&
    validatePricingConfig(invalid1 as any) === false &&
    validatePricingConfig(invalid2 as any) === false &&
    validatePricingConfig(invalid3 as any) === false
  );
}

// Test 5: Tier continuity check
function testTierContinuity(): boolean {
  const config = DEFAULT_PRICING;
...
function testBillableUnitsDeterminism(): boolean {
  const config = DEFAULT_PRICING;
  const weight = 250;
...
function testTierProgression(): boolean {
  const config = DEFAULT_PRICING;
...
function testHashExcludesMetadata(): boolean {
...
// Run all tests
export function runPricingConfigTests(): void {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
