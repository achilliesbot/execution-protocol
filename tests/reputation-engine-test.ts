...
// Test utilities
function createEntry(type: string, sessionId: string = 'test'): VerifierTranscriptEntry {
  return {
...
function createSession(
  outcomeType: 'success' | 'rejected' | 'failed' | 'pending',
  sessionId: string = 'test'
): VerifierTranscriptSession {
  let entries: VerifierTranscriptEntry[] = [];
...
function testExecutionQualityAllSuccess(): boolean {
  const sessions = [
...
function testExecutionQualityMixed(): boolean {
  const sessions = [
    createSession('success'),
...
function testConsistencyHighSuccess(): boolean {
...
function testConsistencyHighFailure(): boolean {
...
function testVolumeIncreases(): boolean {
...
function testPolicyComplianceGood(): boolean {
...
function testPolicyComplianceBad(): boolean {
...
function testComputeReputation(): boolean {
  const sessions = [
  ...
  return (
    reputation.agent_id === 'test-agent' &&
...
function testReputationBounds(): boolean {
...
function testCompareReputation(): boolean {
...
function testEmptySessions(): boolean {
  const reputation = computeReputation('empty-agent', []);
  
  return (
    reputation.score === 50 && // Default neutral score
    reputation.confidence === 0 &&
...
// Run all tests
export function runReputationTests(): void {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
