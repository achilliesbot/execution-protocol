...
import { FeeEstimate } from '../src/fees';

// Test utilities
const TEST_LEDGER_DIR = path.join(process.env.HOME || '/tmp', '.openclaw', 'fees-test');

function createTestFeeEstimate(total: number = 100): FeeEstimate {
  return {
    base_fee: 10,
...
function testDailyRollup(): boolean {
  cleanupTestLedger();
...
function testJsonlDeterminism(): boolean {
  cleanupTestLedger();
...
function testAgentAccumulator(): boolean {
  cleanupTestLedger();
...
function testMonotonicIncrease(): boolean {
  cleanupTestLedger();
...
// Run all tests
export function runFeeLedgerTests(): void {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
