...
function setGasLimit(limit: string): void {
...
function testAllowlistDevnet(): boolean {
...
function testFeatureFlagOff(): Promise<boolean> {
  resetEnv();
  ...
  return submit(request).then((response: any) => {
    resetEnv();
...
function testGasCapEnforcement(): boolean {
...
function testRpcEndpointNone(): boolean {
  resetEnv();
  setNetwork('none');
...
function testRpcEndpointDevnet(): boolean {
  resetEnv();
  setFeatureEnabled(true);
  setNetwork('devnet');
...
function testClientNotReadyWhenDisabled(): boolean {
  resetEnv();
...
function testContractPlaceholder(): boolean {
...
function testPollStatusFeatureFlag(): Promise<boolean> {
  resetEnv();
  setFeatureEnabled(false);
  ...
  return poll('0xabc').then((response: any) => {
    resetEnv();
    return response.status === 'failed' && 
           response.error?.includes('disabled by feature flag');
  });
...
function testVerifyFeatureFlag(): Promise<boolean> {
  resetEnv();
  setFeatureEnabled(false);
  ...
  return verify('attest-123').then((result: boolean) => {
    resetEnv();
    return result === false;
  });
}
...
// Run all tests
export async function runOnChainClientTests(): Promise<void> {
  console.log('\n╔════════════════════════════════════════════════════════════╗');
