 ...
 */
import { countExecutionSteps, calculateComplexity, estimateGas, countComputeUnits, countIOOperations, countPolicyChecks, calculateMetrics, calculateFee, calculateBatchFees, compareFees, getFeeSummary, DEFAULT_FEE_CONFIG } from '../src/fees/index.js';
// Test utilities
...
function testCountExecutionSteps() {
...
function testEmptySessionSteps() {
    const session = { session_id: 'empty', entries: [] };
    return countExecutionSteps(session) === 0;
}
// Test 3: Calculate complexity
function testCalculateComplexity() {
...
function testCountComputeUnits() {
    const session = createBasicSession();
...
function testCountIOOperations() {
    const basic = createBasicSession();
    const complex = createComplexSession();
...
function testCountPolicyChecks() {
    const session = createBasicSession();
...
function testCalculateMetrics() {
    ...
    return (metrics.session_id === 'test-session' &&
...
function testCalculateFeeComplexHigher() {
    const basic = createBasicSession();
    const complex = createComplexSession();
...
function testComplexityMultiplier() {
...
function testGetFeeSummary() {
    const session = createBasicSession();
    const fee = calculateFee(session);
    const summary = getFeeSummary(fee);
    return (summary.includes('Fee Estimate') &&
...
function testDifferentCurrencies() {
    ...
    return (execFee.currency === 'EXEC' &&
        usdcFee.currency === 'USDC' &&
        ethFee.currency === 'ETH');
}
// Test 16: Zero session has zero fee
function testZeroSessionFee() {
    const session = { session_id: 'empty', entries: [] };
    const fee = calculateFee(session);
