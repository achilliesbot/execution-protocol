 ...
 */
import { createAttestationRequest, computeTranscriptHash, generateAttestation, generateMockProof, checkAttestationStatus, verifyAttestationFormat } from '../src/attestation/index.js';
// Test utilities
function createTestSession(entryCount = 2) {
    const entries = [];
...
function testGenerateAttestation() {
    const request = {
    ...
    return (attestation.attestation_id.startsWith('attest-') &&
...
function testGenerateMockProof() {
    const request = {
        transcript_hash: '0xabc123',
        session_id: 'test',
        chain_id: 8453,
    ...
    return (proof.format === 'erc8004-v1' &&
...
function testVerifyInvalidFormatVersion() {
    const proof = {
        format: 'wrong-format',
        transcript_hash: '0xabc123',
        merkle_root: null,
...
function testVerifyMissingSignatures() {
    const proof = {
        format: 'erc8004-v1',
        transcript_hash: '0xabc123',
        merkle_root: null,
...
function testVerifyBadTranscriptHash() {
    const proof = {
        format: 'erc8004-v1',
...
function testAttestationIdDeterminism() {
    const session = createTestSession(2);
...
// Run all tests
export function runAttestationTests() {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
