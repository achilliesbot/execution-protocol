...
// Phase 6 imports
import { calculateBilling } from '../src/billing/index.js';
import { appendRevenueEntry, readRevenueLedger, getRevenueDashboard } from '../src/revenue/index.js';
import { createSettlementAdapter } from '../src/settlement/index.js';
import { calculateMetrics } from '../src/fees/index.js';
import { isMonetizationEnabled, isSettlementEnabled, isSimulationMode } from '../src/config/Phase6Flags.js';
// Load fixture
...
function setPhase6Config(enabled, mode, simulate = true) {
 ...
 */
async function runPhase6Flow(transcript, settlementMode = 'null') {
    ...
    // 2. Calculate billing
    const billing = calculateBilling(transcript.session_id, transcript.entries[transcript.entries.length - 1].entry_hash, metrics);
...
async function testMonetizationDisabled() {
    cleanupLedger();
    resetEnv();
...
async function testSimulateMode() {
    cleanupLedger();
    resetEnv();
...
async function testNoNetworkWrites() {
    cleanupLedger();
    resetEnv();
...
async function testPhase5Compatibility() {
    cleanupLedger();
    resetEnv();
