 ...
 * C. Replay integrity
 */
import { canonicalize, computeHash, computeProposalHash } from '../src/canonicalization/index.js';
import { createPhase1PolicySet, validateAgainstPolicySet } from '../src/policy/index.js';
import { generateExecutionPlan } from '../src/execution/index.js';
...
const createProposalTemplate = (_agent, _session) => JSON.parse(JSON.stringify(FIXED_PROPOSAL));
/**
...
function testCanonicalizationStability() {
    ...
    const hashes = [];
    for (let i = 0; i < 100; i++) {
        hashes.push(computeProposalHash(proposal));
    }
...
function testExecutionPlanDeterminism() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if (result.plan) {
            ...
            const { plan_id, generated_at, ...deterministicParts } = result.plan;
...
function testTranscriptIntegrity() {
    ...
    const proposalEntry = logger.logProposal(sessionId, proposal.proposal_id, computeProposalHash(proposal), 'Swap USDC to ETH', 0.8, 'test-agent', 'claude-sonnet-4.6');
    // Log validation
    const policySet = createPhase1PolicySet();
...
function testCrossSessionDivergence() {
    ...
    const entryA = logger.logProposal(sessionA, proposalA.proposal_id, computeProposalHash(proposalA), 'Swap USDC to ETH', 0.8, 'agent-1', 'claude-sonnet-4.6');
    const entryB = logger.logProposal(sessionB, proposalB.proposal_id, computeProposalHash(proposalB), 'Swap USDC to ETH', 0.8, 'agent-1', 'claude-sonnet-4.6');
...
function testTimestampImmunity() {
    ...
    // Log proposal
    const entry1 = logger.logProposal(sessionId, proposal.proposal_id, computeProposalHash(proposal), 'Swap USDC to ETH', 0.8, 'agent-1', 'claude-sonnet-4.6');
