...
exports.validateProposalStructure = validateProposalStructure;
...
function validateProposalStructure(proposal) {
    ...
    if (proposal.schema_version && proposal.schema_version !== 'v1') {
        errors.push("Invalid schema_version: ".concat(proposal.schema_version, ", expected 'v1'"));
    }
    // Confidence range check
    if (proposal.confidence !== undefined) {
        if (proposal.confidence < 0 || proposal.confidence > 1) {
            errors.push("Invalid confidence: ".concat(proposal.confidence, ", must be 0.0 - 1.0"));
        }
    }
    // Timestamp validity
    if (proposal.timestamp && proposal.expiry) {
        var created = new Date(proposal.timestamp).getTime();
        var expires = new Date(proposal.expiry).getTime();
        if (expires <= created) {
            errors.push('Expiry must be after timestamp');
        }
        // Max 24 hour expiry
        var maxExpiry = created + (24 * 60 * 60 * 1000);
 ...
 */
function createProposalTemplate(agentId, sessionId) {
    var now = new Date();
    var expiry = new Date(now.getTime() + (4 * 60 * 60 * 1000)); // 4 hours
    return {
        schema_version: 'v1',
        proposal_id: "prop_".concat(Date.now(), "_").concat(Math.random().toString(36).substr(2, 9)),
