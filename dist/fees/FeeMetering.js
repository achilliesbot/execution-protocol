 ...
 */
export function countExecutionSteps(session) {
 ...
 * Pure function — no side effects
 */
export function calculateComplexity(session) {
    if (!session.entries || session.entries.length === 0) {
        return 0;
    }
 ...
 * Pure function — no side effects
 */
export function estimateGas(session) {
    if (!session.entries || session.entries.length === 0) {
        return 0;
    }
 ...
 * Pure function — no side effects
 */
export function countComputeUnits(session) {
    if (!session.entries || session.entries.length === 0) {
        return 0;
    }
    let units = 0;
 ...
 * Pure function — no side effects
 */
export function countIOOperations(session) {
    if (!session.entries || session.entries.length === 0) {
        return 0;
    }
 ...
 * Pure function — no side effects
 */
export function countPolicyChecks(session) {
    if (!session.entries || session.entries.length === 0) {
        return 0;
    }
    // Count validation entries
    const validations = session.entries.filter(e => e.entry_type === 'validation_complete').length;
 ...
 * Pure function — no side effects
 */
export function calculateMetrics(session) {
    return {
 ...
 * Pure function — no side effects
 */
export function calculateFee(session, config = DEFAULT_FEE_CONFIG, currency = 'EXEC') {
 ...
 * Pure function — no side effects
 */
export function calculateBatchFees(sessions, config = DEFAULT_FEE_CONFIG, currency = 'EXEC') {
 ...
 * Pure function — no side effects
 */
export function compareFees(a, b) {
 ...
 * Pure function — no side effects
 */
export function getFeeSummary(estimate) {
    return `
...
//# sourceMappingURL=FeeMetering.js.map